// 在线版版的
const fs = require('fs');
let mainTab = {js: "", classArr: [], 头文件数组:[]}; // 输出这个
...
let 加载普通类 = `
        if (this.#名字# && data[#id#]) {
            this.#名字#.zzzzz_load(data[#id#], #id#, "#名字#", "#名字#");
            this.#名字#.zzzzz_model = this;    
        }
        else if (!this.#名字# && data[#id#]) {
            this.#名字# = new #类名#();
            this.#名字#.zzzzz_load(data[#id#], #id#, "#名字#")
            this.#名字#.zzzzz_model = this; 
...
export class Model #继承# {
    ...
    constructor () {
        _self = this;
        let data = // 网络上获取data, 这里不填故意报错
        this.zzzzz_load(data);
    }
    private zzzzz_load (data) {
        this._data = data;
#加载普通类#
...
function mainclass(继承, ...dataArr) { // 添加子类, 子类名, 到时候要处理的 // TODO:mainclass
    ...
    for (let i = 0; i < 头文件数组.length; i++) {
        let str = ("import { 类名 } from './类名';\n").replace(/名字/g, 头文件数组[i]);
        头文件 += str;
    }
    // fs.writeFile('../assets/Script/Model.ts', 头文件 +  mainTab.js + js, (err) => {
